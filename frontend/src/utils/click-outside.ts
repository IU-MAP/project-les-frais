...
const beforeMount = (el: ClickOutsideElement, { value }: DirectiveBinding) => {
  ...
  el[HANDLERS_PROPERTY] = events.map((eventName) => ({
    ...
    handler: (event: MouseEvent) => onEvent({
      el, event, handler, middleware,
    }),
    capture,
  }));
  ...
  if (detectIframe) {
    const detectIframeEvent = {
      event: 'blur',
      srcTarget: window,
      handler: (event: MouseEvent) => onFauxIframeClick({
        el, event, handler, middleware,
      }),
      capture,
    };
  ...
  el[HANDLERS_PROPERTY].forEach(
    ({ event, srcTarget, handler: thisHandler }: HandlerObject) => setTimeout(() => {
...
const unmounted = (el: ClickOutsideElement) => {
  ...
  handlers.forEach(({
    event, srcTarget, handler, capture,
